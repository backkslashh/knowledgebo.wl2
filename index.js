...
function newQuestion(callback, room) {
  ...
  callback(
    JSON.stringify([
      {
        ID: 107103,
        Subject: "Atomic Subtraction",
        Question:
          "Subtract the atomic number of lithium from the atomic number of uranium.",
        Answer: "89",
        Metadata:
          "atomic number chemistry element lithium periodic table physical science science uranium",
      },
    ])
  );
}
  ...
  roomsInfo[room]["endOfAnswerInterval"] = setInterval(function () {
    if (roomsInfo[room]["endTimeRemaining"] > 0) {
      ...
      io.to(room).emit(
        ...
        roomsInfo[room]["endTimeRemaining"]
      );
    } else {
      roomsInfo[room]["questionDone"] = true;
      if (
        roomsInfo[room]["lastSentAnswer"] ===
          roomsInfo[room]["currentQuestion"]["Answer"] ||
        ...
        if (roomsInfo[room]["canBuzz"]) {
          ...
          roomsInfo[room]["answeringInterval"] = setInterval(function () {
            if (roomsInfo[room]["answerTimeRemaining"] !== 0) {
              io.to(room).emit(
                "answer countdown",
                roomsInfo[room]["answerTimeRemaining"]
              );
...
function resumeReading(room) {
  ...
  roomsInfo[room]["resumeReadingInterval"] = setInterval(function () {
    if (roomsInfo[room]["unreadQuestion"].length !== 0) {
      roomsInfo[room]["readQuestion"] +=
        roomsInfo[room]["unreadQuestion"][0] + " ";
